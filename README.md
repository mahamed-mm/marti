...
Current phase, shipped features, active blockers, and what's next: [`docs/STATUS.md`](docs/STATUS.md).
...
3. Run the SQL files against your Supabase project, in order:
   - `docs/db/001_listings.sql`
   - `docs/db/002_sample_listings.sql`
   - `docs/db/003_categories.sql`
   - `docs/db/004_sample_categories.sql`
...
```
marti/Marti/
├── MartiApp.swift          — @main; configures Supabase, Mapbox, ModelContainer
├── Info.plist              — SUPABASE_URL / SUPABASE_ANON_KEY / MBXAccessToken
├── Assets.xcassets
├── Models/                 — Pure value types + paired @Model/DTO
├── Services/               — Protocol + concrete impl, one file per concern
...
marti/MartiTests/           — Mirrors source folder structure
marti/MartiUITests/         — UI tests (flaky in CI; unit-test first)

docs/
├── PRD.md                  — Product requirements
├── ARCHITECTURE.md         — Architecture spec (hand-maintained)
├── DESIGN.md               — Design-system spec (hand-maintained)
├── STATUS.md               — Current phase, shipped, blockers, what's next
